...
def load_IPD(scheme_id=1, prefix=''):

    if scheme_id == 1:  # iterative prisoner's dilemma
        def reward_from_A(N):  # R
            return [3, 0]

        def reward_from_B(N):  # S
            return [0, 0]

        def reward_from_C(N):  # T
            return [5, 0]

        def reward_from_D(N):  # P
            return [1, 0]

        fig_name = './figures/'+prefix+'IPD_1'

    elif scheme_id == 2:  # chicken game
        def reward_from_A(N):
            return [6, 0]
...
#         pfC2 = np.random.random()
        ...
        label_B = 'left: trimodal N('+str(p1mB)+','+str(p1sB)+'):N('+str(p2mB)+','+str(p2sB)+'):N('+str(
            p3mB)+','+str(p3sB)+') = {0:.2f}'.format(pfB1)+':{0:.2f}'.format(pfB2-pfB1)+':{0:.2f}'.format(1-pfB2)
        ...
        label_C = 'right: trimodal N('+str(p1mC)+','+str(p1sC)+'):N('+str(p2mC)+','+str(p2sC)+'):N('+str(
            p3mC)+','+str(p3sC)+') = {0:.2f}'.format(pfC1)+':{0:.2f}'.format(pfC2-pfC1)+':{0:.2f}'.format(1-pfC2)
...
#         pfC2 = np.random.random()
        ...
        def reward_from_B():
            ...
            if (p < pfB):
                ...
                return np.random.normal(p2mB, p2sB)
        label_B = 'left: bimodal N('+str(p1mB)+','+str(p1sB)+'):N('+str(
            p2mB)+','+str(p2sB)+') = {0:.2f}'.format(pfB)+':{0:.2f}'.format(1-pfB)
        ...
        def reward_from_C():
            ...
            if (p < pfC):
                ...
                return np.random.normal(p2mC, p2sC)
        label_C = 'right: bimodal N('+str(p1mC)+','+str(p1sC)+'):N('+str(
            p2mC)+','+str(p2sC)+') = {0:.2f}'.format(pfC)+':{0:.2f}'.format(1-pfC)
        ...
        label_B = 'left: 4-modal N('+str(p1mB)+','+str(p1sB)+'):N('+str(p2mB)+','+str(p2sB)+'):N('+str(p3mB)+','+str(p3sB)+'):N('+str(
            p4mB)+','+str(p4sB)+') = {0:.2f}'.format(pfB1)+':{0:.2f}'.format(pfB2-pfB1)+':{0:.2f}'.format(pfB3-pfB2)+':{0:.2f}'.format(1-pfB3)
        ...
        label_C = 'right: 4-modal N('+str(p1mC)+','+str(p1sC)+'):N('+str(p2mC)+','+str(p2sC)+'):N('+str(p3mC)+','+str(p3sC)+'):N('+str(
            p4mC)+','+str(p4sC)+') = {0:.2f}'.format(pfC1)+':{0:.2f}'.format(pfC2-pfC1)+':{0:.2f}'.format(pfC3-pfC2)+':{0:.2f}'.format(1-pfC3)

        fig_name = './figures/'+prefix+''+keyword+'_'+str(scenario_id)

    if is_flipped:
        reward_from_C, reward_from_B, label_C, label_B = reward_from_B, reward_from_C, label_B, label_C
        fig_name = './figures/'+prefix+'' + \
            keyword+'_'+str(scenario_id)+'_flipped'
    fig_name_flipped = './figures/'+prefix+'' + \
        keyword+'_'+str(scenario_id)+'_flipped'
    kld, crossentropy, sB, sC, sdiff = get_dist_stats(
        reward_from_B, reward_from_C)
...
def collateComparison(nameA, nameB, scenario, scoreA, scoreB, sB, sC, sdiff, crossentropy):
    ...
    crossentropyAoBm, crossentropyAoBs = np.mean(
        crossentropyAoB), np.std(crossentropyAoB)
    ...
    crossentropyBoAm, crossentropyBoAs = np.mean(
        crossentropyBoA), np.std(crossentropyBoA)

    print('final scores for ', nameA, 'and', nameB, ':')
    print(scoreAm, '+/-', scoreAs, ' vs. ',
          scoreBm, '+/-', scoreBs, ', p=', scorep)
    print('\n')
    ...
    print(sdiffAoBm, '+/-', sdiffAoBs, ' vs. ',
          sdiffBoAm, '+/-', sdiffBoAs, ', p=', sdiffp)
    print('crossentropy for the winning cases for ', nameA, 'and', nameB, ':')
    print(crossentropyAoBm, '+/-', crossentropyAoBs, ' vs. ',
          crossentropyBoAm, '+/-', crossentropyBoAs, ', p=', crossentropyp)
