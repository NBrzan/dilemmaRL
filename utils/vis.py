...
def run_ipd(ipd_scenario, algs, nMemory=5, nTrials=100, T=160, prefix="", reputations=None, rep_counts=None, enable_reputation=False):
    ...
    ipd_case = IPD(algs, reward_functions, nTrials, T, nMemory=nMemory, reputations=reputations,
                   rep_counts=rep_counts, enable_reputation=enable_reputation, clear_feats=False)
    rep = ipd_case.run()
    rep['algs'] = algs
    rep['nTrials'] = nTrials
    rep['T'] = T
    ...
    rep['max_r'] = np.max([np.sum(reward_from_A(0)), np.sum(
        reward_from_B(0)), np.sum(reward_from_C(0)), np.sum(reward_from_D(0))])
    fig_r = './figures/'+prefix+'/r_IPD_' + \
        str(ipd_scenario)+'_m_'+str(nMemory)+'_' + '_'.join(algs)
    fig_f = './figures/'+prefix+'/f_IPD_' + \
        str(ipd_scenario)+'_m_'+str(nMemory)+'_' + '_'.join(algs)
    fig_p = './figures/'+prefix+'/p_IPD_' + \
        str(ipd_scenario)+'_m_'+str(nMemory)+'_' + '_'.join(algs)
    plot_r(rep, fig_r)
    ...
    model_path = './models/'+prefix+'/IPD_' + \
        str(ipd_scenario)+'_m_'+str(nMemory)+'_p_' + '_'.join(algs)+'.csv'
...
def plot_r(rep, filename, cum=True, legend=True):
    ...
    for idx in np.arange(len(avg_mean)):
        ax1.plot(avg_mean[idx], alpha=alpha1, lw=LINE_WIDTH,
                 label=name[idx], c=colors[name[idx]])
    if avg_std is not None:
        for idx in np.arange(len(avg_mean)):
            m = idx
            ax1.fill_between(np.arange(len(avg_mean[m])), avg_mean[m] - avg_std[m]/np.sqrt(
                nT), avg_mean[m] + avg_std[m]/np.sqrt(nT), alpha=alpha2, color=colors[name[m]])
    ax1.set_xlabel('Krog')
    if legend:
        ax1.legend()
    if cum:
        ...
        ax1.title.set_text(
            'Povratna informacija o nagradi: '+' proti '.join(name))
        ax1.set_ylim([0, 1.5])
    ax1.set_xlim([0, T])
...
def plotAgents(reports, names, nTrials, reward_functions, labels, fig_name, is_flipped=False, isIGT=False, plotShortTerm=False, shortTerm=100):
    ...
    if isIGT:
        ...
        for rep in reports:
            if is_flipped:
                y.append(rep['percent'])
            else:
                y.append(100-rep['percent'])
        ...
        if plotShortTerm:
            ...
            axlist = [[ax1, ax2, ax2b, ax3, ax3b], [
                ax4, ax5, ax6, ax7], [ax8, ax9, ax10, ax11]]
        else:
            ax1 = plt.subplot2grid((3, 12), (0, 0), colspan=4)
        ...
        axlist[0][0].axvline(x=np.mean(data['A']),
                             color='blue', linestyle='--')
        axlist[0][0].axvline(x=np.mean(data['B']),
                             color='orange', linestyle='--')
        axlist[0][0].axvline(x=np.mean(data['C']),
        ...
        axlist[0][0].legend(loc='upper center',
                            bbox_to_anchor=(0.1, 0.8), ncol=1)
        axlist[0][0].grid(True)

        if plotShortTerm:
            last = 10
            for i, _ in enumerate(y):
                l, = axlist[0][1].plot(steps[:shortTerm], np.mean(
                    y[i], 0)[:shortTerm], marker='', color=colors[i], linewidth=lw, label=names[i])
                l, = axlist[0][2].plot(steps[-last:], np.mean(y[i], 0)[-last:],
                                       marker='', color=colors[i], linewidth=lw, label=names[i])
                l, = axlist[0][3].plot(steps[:shortTerm], np.mean(
                    r[i], 0)[:shortTerm], marker='', color=colors[i], linewidth=lw, label=names[i])
                l, = axlist[0][4].plot(steps[-last:], np.mean(r[i], 0)[-last:],
                                       marker='', color=colors[i], linewidth=lw, label=names[i])
                l = axlist[0][1].fill_between(steps[:shortTerm], np.mean(y[i], 0)[:shortTerm]-np.std(y[i], 0)[:shortTerm]/math.sqrt(
                    nTrials), np.mean(y[i], 0)[:shortTerm]+np.std(y[i], 0)[:shortTerm]/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
                l = axlist[0][2].fill_between(steps[-last:], np.mean(y[i], 0)[-last:]-np.std(y[i], 0)[-last:]/math.sqrt(
                    nTrials), np.mean(y[i], 0)[-last:]+np.std(y[i], 0)[-last:]/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
                l = axlist[0][3].fill_between(steps[:shortTerm], np.mean(r[i], 0)[:shortTerm]-np.std(r[i], 0)[:shortTerm]/math.sqrt(
                    nTrials), np.mean(r[i], 0)[:shortTerm]+np.std(r[i], 0)[:shortTerm]/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
                l = axlist[0][4].fill_between(steps[-last:], np.mean(r[i], 0)[-last:]-np.std(r[i], 0)[-last:]/math.sqrt(
                    nTrials), np.mean(r[i], 0)[-last:]+np.std(r[i], 0)[-last:]/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
...
#             axlist[0][2].yaxis.tick_right()
            ...
            for i, _ in enumerate(y):
                l, = axlist[0][1].plot(steps, np.mean(
                    y[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
                l, = axlist[0][2].plot(steps, np.mean(
                    r[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
                l = axlist[0][1].fill_between(steps, np.mean(y[i], 0)-np.std(y[i], 0)/math.sqrt(nTrials), np.mean(
                    y[i], 0)+np.std(y[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
                l = axlist[0][2].fill_between(steps, np.mean(r[i], 0)-np.std(r[i], 0)/math.sqrt(nTrials), np.mean(
                    r[i], 0)+np.std(r[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            axlist[0][1].set_xlabel('Epizode')
        ...
        for i, _ in enumerate(y):
            l, = axlist[1][0].plot(steps, np.mean(
                a1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[1][1].plot(steps, np.mean(
                b1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[1][2].plot(steps, np.mean(
                c1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[1][3].plot(steps, np.mean(
                d1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[2][0].plot(steps, np.mean(
                a2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[2][1].plot(steps, np.mean(
                b2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[2][2].plot(steps, np.mean(
                c2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l, = axlist[2][3].plot(steps, np.mean(
                d2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[1][0].fill_between(steps, np.mean(a1[i], 0)-np.std(a1[i], 0)/math.sqrt(nTrials), np.mean(
                a1[i], 0)+np.std(a1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[1][1].fill_between(steps, np.mean(b1[i], 0)-np.std(b1[i], 0)/math.sqrt(nTrials), np.mean(
                b1[i], 0)+np.std(b1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[1][2].fill_between(steps, np.mean(c1[i], 0)-np.std(c1[i], 0)/math.sqrt(nTrials), np.mean(
                c1[i], 0)+np.std(c1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[1][3].fill_between(steps, np.mean(d1[i], 0)-np.std(d1[i], 0)/math.sqrt(nTrials), np.mean(
                d1[i], 0)+np.std(d1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[2][0].fill_between(steps, np.mean(a2[i], 0)-np.std(a2[i], 0)/math.sqrt(nTrials), np.mean(
                a2[i], 0)+np.std(a2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[2][1].fill_between(steps, np.mean(b2[i], 0)-np.std(b2[i], 0)/math.sqrt(nTrials), np.mean(
                b2[i], 0)+np.std(b2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[2][2].fill_between(steps, np.mean(c2[i], 0)-np.std(c2[i], 0)/math.sqrt(nTrials), np.mean(
                c2[i], 0)+np.std(c2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l = axlist[2][3].fill_between(steps, np.mean(d2[i], 0)-np.std(d2[i], 0)/math.sqrt(nTrials), np.mean(
                d2[i], 0)+np.std(d2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
        ...
        axlist[0][0].axvline(x=np.mean(data[label_B]),
                             color='blue', linestyle='--')
        axlist[0][0].axvline(x=np.mean(data[label_C]),
                             color='orange', linestyle='--')
        axlist[0][0].set_xlabel('Nagrada')
        axlist[0][0].set_ylabel(
            'Porazdelitev nagrad za dejanje levo proti desno')
        ...
        for i, _ in enumerate(y):
            ...
            l = axlist[0][1].fill_between(steps, np.mean(y[i], 0)-np.std(y[i], 0)/math.sqrt(nTrials), np.mean(
                y[i], 0)+np.std(y[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l, = axlist[0][2].plot(steps, np.mean(
                c[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[0][2].fill_between(steps, np.mean(c[i], 0)-np.std(c[i], 0)/math.sqrt(nTrials), np.mean(
                c[i], 0)+np.std(c[i], 0) / math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l, = axlist[1][0].plot(steps, np.mean(
                l1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[1][0].fill_between(steps, np.mean(l1[i], 0)-np.std(l1[i], 0)/math.sqrt(nTrials), np.mean(
                l1[i], 0)+np.std(l1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l, = axlist[1][1].plot(steps, np.mean(
                r1[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[1][1].fill_between(steps, np.mean(r1[i], 0)-np.std(r1[i], 0)/math.sqrt(nTrials), np.mean(
                r1[i], 0)+np.std(r1[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l, = axlist[1][2].plot(steps, np.mean(
                l2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[1][2].fill_between(steps, np.mean(l2[i], 0)-np.std(l2[i], 0)/math.sqrt(nTrials), np.mean(
                l2[i], 0)+np.std(l2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
            l, = axlist[1][3].plot(steps, np.mean(
                r2[i], 0), marker='', color=colors[i], linewidth=lw, label=names[i])
            l = axlist[1][3].fill_between(steps, np.mean(r2[i], 0)-np.std(r2[i], 0)/math.sqrt(nTrials), np.mean(
                r2[i], 0)+np.std(r2[i], 0)/math.sqrt(nTrials), alpha=alpha_plot, color=colors[i])
        axlist[0][1].set_xlabel('Epizode')
