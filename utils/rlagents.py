...
class MDP():
    ...
    def __init__(self, algorithm, reward_functions, nTrials, T, nAct_B=20, nAct_C=20, GAMMA=0.95, Q1=None, Q2=None, Q1s=None, Q2s=None, Traj=None):
        ...
        self.actionsPerState[self.STATE_A] = [
        ...
        self.stateSpace = [self.STATE_A, self.STATE_B,
                           self.STATE_C, self.STATE_D, self.STATE_E]
        self.fQprime = None
        ...
        for s in self.stateSpace:
            ...
            if N[s] < len(self.actionsPerState[s]):
                ...
                for a in self.actionsPerState[s]:
                    ...
                    Qprime[s][a] = theta.T.dot(self.feats.flatten()) + alpha*np.sqrt(
                        self.feats.flatten().T.dot(np.linalg.inv(A[s][a])).dot(self.feats.flatten()))
        return Qprime
    ...
    def getQCTS(self, Q, NSA, player=None):
        ...
        for s in self.stateSpace:
            ...
            for a in self.actionsPerState[s]:
                B, f = Q[s][a][0], Q[s][a][1]
                try:
                    Qprime[s][a] = self.feats.flatten().T.dot(np.random.multivariate_normal(
                        np.linalg.inv(B).dot(f).flatten(), v2 * np.linalg.inv(B)))
                except:
                    Qprime[s][a] = self.feats.flatten().T.dot(np.random.multivariate_normal(np.linalg.inv(
                        B+np.eye(len(self.feats.flatten()))).dot(f).flatten(), v2 * np.linalg.inv(B+np.eye(len(self.feats.flatten())))))
        return Qprime
    ...
    def getQEXP3(self, mu, player=None):
        ...
        for s in self.stateSpace:
            ...
            for a in self.actionsPerState[s]:
                Qprime[s][a] = (
                    1-mu) * Q1[s][a] / np.sum(list(Q1[s].values())) + mu / len(self.actionsPerState[s])
        return Qprime
        ...
        for s in self.stateSpace:
            ...
            for a in self.actionsPerState[s]:
                if self.algorithm in ['LinUCB']:
                    ...
                    self.Q1[s][a] = np.ones(
                        (len(self.feats.flatten()), 1))/len(self.feats.flatten())
                    self.Q2[s][a] = np.zeros((len(self.feats.flatten()), 1))
                elif self.algorithm in ['CTS', 'SCTS', 'PCTS', 'NCTS', 'cADD', 'cADHD', 'cAD', 'cCP', 'cbvFTD', 'cPD', 'cM']:
                    self.Q1[s][a] = [np.eye(len(self.feats.flatten())), np.zeros(
                        (len(self.feats.flatten()), 1))]
                    self.Q2[s][a] = [np.eye(len(self.feats.flatten())), np.zeros(
                        (len(self.feats.flatten()), 1))]
                else:
    ...
    def randomAction(self, s, a, eps=.1):
        ...
        if p < (1 - eps):
            ...
            return np.random.choice(self.actionsPerState[s])
        ...
        if self.BC and self.trajs is not None:
            if (s == self.STATE_A):
                ...
                if (a == a_cor):
                    ...
                    return 0, a_cor
            if s == self.STATE_B:
                return 1, self.STATE_D
            if s == self.STATE_C:
                return 1, self.STATE_E
        else:
            if (s == self.STATE_A):
                if (a == self.ACTION_LEFT):
                    return 0, self.STATE_B
                elif (a == self.ACTION_RIGHT):
                    return 0, self.STATE_C
            if s == self.STATE_B:
                return self.reward_from_B(), self.STATE_D
            if s == self.STATE_C:
                return self.reward_from_C(), self.STATE_E
        return 0, s
    ...
    def experiment(self):
        ...
        for i in range(self.T):
            ...
            s = self.initialState
            if i == 0:
                a = self.selectInitialAction(
                    self.initialState, True, last_a, N, NSA, t=i)
            else:
                a = self.selectInitialAction(
                    self.initialState, False, last_a, N, NSA, t=i)
            ...
            while not gameover:
                ...
                if self.algorithm in ['DQL', 'QL', 'SARSA', 'MP', 'SQL', 'SQL2', 'PQL', 'NQL', 'MP', 'ESQL', 'DSQL', 'ADD', 'ADHD', 'AD', 'CP', 'bvFTD', 'PD', 'M']:
                    ...
                    a = self.randomAction(s, a, self.epsilon)
                ...
                self.feats = np.zeros((len(self.stateSpace), 1))
                self.feats[s] = 1
                if (s == self.STATE_A and a == self.ACTION_LEFT):
                    ALeft += 1
                ...
                reward = r
                if r > 0:
                    pos_reward, neg_reward = r, 0
                else:
                    pos_reward, neg_reward = 0, r
#                 reward.append(r)
                ...
                nxt_a = self.updateAgent(
                    s, a, r, nxt_s, alpha, t, N=N, NSA=NSA)
...
#             episodes[i]["cumreward"] = sum(reward)
            episodes[i]["Q1(A)l"] = (episodes[i]["Q1(A)l"] *
                                     i + self.Q1[self.STATE_A][self.ACTION_LEFT])/(i+1)
            episodes[i]["Q2(A)l"] = (episodes[i]["Q2(A)l"] *
                                     i + self.Q2[self.STATE_A][self.ACTION_LEFT])/(i+1)
            episodes[i]["Q1(A)r"] = (episodes[i]["Q1(A)r"] *
                                     i + self.Q1[self.STATE_A][self.ACTION_RIGHT])/(i+1)
            episodes[i]["Q2(A)r"] = (episodes[i]["Q2(A)r"] *
                                     i + self.Q2[self.STATE_A][self.ACTION_RIGHT])/(i+1)
    ...
    def run(self):
        ...
        cumreward = pos_reward = neg_reward = actions = np.ndarray(
            (self.nTrials, self.T))
...
#         report["cumreward"] = cumreward
    ...
    def getBias(self, alg=None):
        if alg is None:
            alg = self.algorithm
        if alg in ['SQL', 'SQL2', 'HBTS', 'MP', 'ESQL', 'EHBTS', 'SCTS']:
            p1, p2, n1, n2 = 1, 1, 1, 1
        elif alg in ['PQL', 'PTS', 'PCTS']:
            p1, p2, n1, n2 = 1, 1, 0, 0
        elif alg in ['NQL', 'NTS', 'NCTS']:
            p1, p2, n1, n2 = 0, 0, 1, 1
        elif alg in ['ADD', 'bADD', 'cADD']:
            p1, p2, n1, n2 = np.random.normal(1, 0.1), np.random.normal(
                1, 0.1), np.random.normal(0.5, 0.1), np.random.normal(1, 0.1)
        elif alg in ['ADHD', 'bADHD', 'cADHD']:
            p1, p2, n1, n2 = np.random.normal(0.2, 0.1), np.random.normal(
                1, 0.1), np.random.normal(0.2, 0.1), np.random.normal(1, 0.1)
        elif alg in ['AD', 'bAD', 'cAD']:
            p1, p2, n1, n2 = np.random.normal(0.1, 0.1), np.random.normal(
                1, 0.1), np.random.normal(0.1, 0.1), np.random.normal(1, 0.1)
        elif alg in ['CP', 'bCP', 'cCP']:
            p1, p2, n1, n2 = np.random.normal(0.5, 0.1), np.random.normal(
                0.5, 0.1), np.random.normal(1, 0.1), np.random.normal(1, 0.1)
        elif alg in ['bvFTD', 'bbvFTD', 'cbvFTD']:
            p1, p2, n1, n2 = np.random.normal(0.5, 0.1), np.random.normal(
                100, 10), np.random.normal(0.5, 0.1), np.random.normal(1, 0.1)
        elif alg in ['PD', 'bPD', 'cPD']:
            p1, p2, n1, n2 = np.random.normal(0.5, 0.1), np.random.normal(
                1, 0.1), np.random.normal(0.5, 0.1), np.random.normal(100, 10)
        elif alg in ['M', 'bM', 'cM']:
            p1, p2, n1, n2 = np.random.normal(0.5, 0.1), np.random.normal(
                1, 0.1), np.random.normal(0.5, 0.1), np.random.normal(1, 0.1)
        else:
    ...
    def act(self, s, last_a, N, NSA, alg, fQprime, Q1, Q2, player=None, t=None):
        ...
        if alg in ['Tit4Tat']:
            nxt_a = self.ACTION_RIGHT if - \
                1 in list(self.feats[np.arange(self.nP) != player, 0]) else self.ACTION_LEFT
    ...
    def updateAgent(self, s, a, r, nxt_s, alpha, t, pr=None, nr=None, N=None, NSA=None, alg=None, player=None, fQprime=None):
        ...
        nxt_a, maxq, isQ1forDQL = self.act(
            nxt_s, a, N, NSA, alg, fQprime, Q1, Q2, player, t)

        if alg == 'SARSA':
            # Q1[s][a] = Q1[s][a] + alpha * (r + self.GAMMA * self.getExpected(Q1s) - Q1[s][a])
            Q1[s][a] = Q1[s][a] + alpha * \
                (r + self.GAMMA * Q1[nxt_s][nxt_a] - Q1[s][a])
        ...
        if alg == 'DQL':
            ...
            if isQ1forDQL:
                Q1[s][a] = Q1[s][a] + alpha * \
                    (r + self.GAMMA * Q2[nxt_s][nxt_a] - Q1[s][a])
            else:
                Q2[s][a] = Q2[s][a] + alpha * \
                    (r + self.GAMMA * Q1[nxt_s][nxt_a] - Q2[s][a])

        if alg == 'MP':
            if pr is not None and nr is not None:
                Q1[s][a] = p1*Q1[s][a] + alpha * \
                    (p2*pr + self.GAMMA * Q1[nxt_s][nxt_a] - Q1[s][a])
                Q2[s][a] = n1*Q2[s][a] + alpha * \
                    (n2*nr + self.GAMMA * Q2[nxt_s][nxt_a] - Q2[s][a])
            else:
                if (r >= 0):
                    Q1[s][a] = p1*Q1[s][a] + alpha * \
                        (p2*r + self.GAMMA * Q1[nxt_s][nxt_a] - Q1[s][a])
                if (r <= 0):
                    Q2[s][a] = n1*Q2[s][a] + alpha * \
                        (n2*r + self.GAMMA * Q2[nxt_s][nxt_a] - Q2[s][a])
        ...
        if alg == 'ESQL':
            ...
            if pr is not None and nr is not None:
                try:
                    ...
                    pr = pr
                Q1[s][a] = p1*Q1[s][a] + alpha * \
                    (p2*pr + self.GAMMA * Q1[nxt_s][nxt_a1] - Q1[s][a])
                try:
                    nr = nr*math.exp(-nr/rho)
                except:
                    nr = nr
                Q2[s][a] = n1*Q2[s][a] + alpha * \
                    (n2*nr + self.GAMMA * Q2[nxt_s][nxt_a2] - Q2[s][a])
            else:
                if (r >= 0):
                    try:
                        r = r*math.exp(r/rho)
                        ...
                        r = r
                    Q1[s][a] = p1*Q1[s][a] + alpha * \
                        (p2*r + self.GAMMA * Q1[nxt_s][nxt_a1] - Q1[s][a])
                if (r <= 0):
                    try:
                        r = r*math.exp(-r/rho)
                    except:
                        r = r
                    Q2[s][a] = n1*Q2[s][a] + alpha * \
                        (n2*r + self.GAMMA * Q2[nxt_s][nxt_a2] - Q2[s][a])
        ...
        if alg in ['SQL2']:
            ...
            if pr is not None and nr is not None:
                Q1[s][a] = p1*Q1[s][a] + alpha * \
                    (pr + self.GAMMA * Q1[nxt_s][nxt_a1] - Q1[s][a])
                Q2[s][a] = n1*Q2[s][a] + alpha * \
                    (nr + self.GAMMA * Q2[nxt_s][nxt_a2] - Q2[s][a])
            else:
                if (r >= 0):
                    Q1[s][a] = p1*Q1[s][a] + alpha * \
                        (r + self.GAMMA * Q1[nxt_s][nxt_a1] - Q1[s][a])
                if (r <= 0):
                    Q2[s][a] = n1*Q2[s][a] + alpha * \
                        (r + self.GAMMA * Q2[nxt_s][nxt_a2] - Q2[s][a])
        ...
        if alg in ['LinUCB']:
            ...
            Q2[s][a] = Q2[s][a] + r * \
                self.feats.flatten().reshape((len(self.feats.flatten()), 1))

        if alg in ['CTS']:
            Q1[s][a] = [Q1[s][a][0] + self.feats.flatten().dot(self.feats.flatten().T),
                        Q1[s][a][1] + self.feats.flatten().reshape((len(self.feats.flatten()), 1))*r]

        if alg in ['SCTS', 'PCTS', 'NCTS', 'cADD', 'cADHD', 'cAD', 'cCP', 'cbvFTD', 'cPD', 'cM']:
            if pr is not None and nr is not None:
                Q1[s][a] = [p1*Q1[s][a][0] + self.feats.flatten().dot(self.feats.flatten().T), p1 *
                            Q1[s][a][1] + p2*self.feats.flatten().reshape((len(self.feats.flatten()), 1))*pr]
                Q2[s][a] = [n1*Q2[s][a][0] + self.feats.flatten().dot(self.feats.flatten().T), n1 *
                            Q2[s][a][1] + n2*self.feats.flatten().reshape((len(self.feats.flatten()), 1))*nr]
            else:
                if (r >= 0):
                    Q1[s][a] = [p1*Q1[s][a][0] + self.feats.flatten().dot(self.feats.flatten().T), p1 *
                                Q1[s][a][1] + p2*self.feats.flatten().reshape((len(self.feats.flatten()), 1))*r]
                if (r <= 0):
                    Q2[s][a] = [n1*Q2[s][a][0] + self.feats.flatten().dot(self.feats.flatten().T), n1 *
                                Q2[s][a][1] + n2*self.feats.flatten().reshape((len(self.feats.flatten()), 1))*r]
...
class MAB(MDP):
    ...
    def __init__(self, algorithm, reward_functions, nTrials, T, Q1=None, Q2=None, Q1s=None, Q2s=None, Traj=None):
        MDP.__init__(self, algorithm, reward_functions, nTrials,
                     T, Q1=Q1, Q2=Q2, Q1s=Q1s, Q2s=Q2s, Traj=Traj)
    ...
    def move(self, s, a, t=None):
        if self.BC and self.trajs is not None:
            ...
            if (s == self.STATE_A):
                if (a == self.ACTION_LEFT):
                    return self.reward_from_B(), self.STATE_B
                elif (a == self.ACTION_RIGHT):
                    return self.reward_from_C(), self.STATE_C
            else:
                return 0, s
...
class IGT(MDP):
    ...
    def __init__(self, algorithm, reward_functions, nTrials, T, Q1=None, Q2=None, Q1s=None, Q2s=None, Traj=None):
        ...
        # map actions to states
        self.actionsPerState = {}
        self.actionsPerState[self.STATE_E] = [
            self.ACTION_A, self.ACTION_B, self.ACTION_C, self.ACTION_D]
    ...
    def move(self, s, a, N, t=None):
        if self.BC and self.trajs is not None:
            ...
            if (s == self.STATE_E):
                if (a == self.ACTION_A):
                    return self.reward_from_A(N[self.STATE_A]), self.STATE_A
                elif (a == self.ACTION_B):
                    return self.reward_from_B(N[self.STATE_B]), self.STATE_B
                elif (a == self.ACTION_C):
                    return self.reward_from_C(N[self.STATE_C]), self.STATE_C
                elif (a == self.ACTION_D):
                    return self.reward_from_D(N[self.STATE_D]), self.STATE_D
        return [0, 0], s
    ...
    def experiment(self):
        ...
        for i in range(self.T):
            ...
            while not gameover:
                ...
                nxt_a = self.updateAgent(
                    s, a, r, nxt_s, alpha, t, pr, nr, N, NSA)
            ...
            episodes[i]["actions"] = actions
#             episodes[i]["cumreward"] = sum(reward)
            episodes[i]["Q1(I)a"] = ((episodes[i]["Q1(I)a"] * i) +
                                     self.Q1[self.STATE_E][self.ACTION_A])/(i+1)
            episodes[i]["Q2(I)a"] = ((episodes[i]["Q2(I)a"] * i) +
                                     self.Q2[self.STATE_E][self.ACTION_A])/(i+1)
            episodes[i]["Q1(I)b"] = ((episodes[i]["Q1(I)b"] * i) +
                                     self.Q1[self.STATE_E][self.ACTION_B])/(i+1)
            episodes[i]["Q2(I)b"] = ((episodes[i]["Q2(I)b"] * i) +
                                     self.Q2[self.STATE_E][self.ACTION_B])/(i+1)
            episodes[i]["Q1(I)c"] = ((episodes[i]["Q1(I)c"] * i) +
                                     self.Q1[self.STATE_E][self.ACTION_C])/(i+1)
            episodes[i]["Q2(I)c"] = ((episodes[i]["Q2(I)c"] * i) +
                                     self.Q2[self.STATE_E][self.ACTION_C])/(i+1)
            episodes[i]["Q1(I)d"] = ((episodes[i]["Q1(I)d"] * i) +
                                     self.Q1[self.STATE_E][self.ACTION_D])/(i+1)
            episodes[i]["Q2(I)d"] = ((episodes[i]["Q2(I)d"] * i) +
                                     self.Q2[self.STATE_E][self.ACTION_D])/(i+1)
    ...
    def run(self):
        ...
        cumreward = reward = pos_reward = neg_reward = actions = np.ndarray(
            (self.nTrials, self.T))
...
class IPD(MDP):
            ...
            for s in self.stateSpace:
                ...
                for a in self.actionsPerState[s]:
                    if alg in ['LinUCB']:
                        ...
                        Q1[s][a] = [np.eye(len(self.feats.flatten())), np.zeros(
                            (len(self.feats.flatten()), 1))]
                        Q2[s][a] = [np.eye(len(self.feats.flatten())), np.zeros(
                            (len(self.feats.flatten()), 1))]
                    else:
                        Q1[s][a] = defaultQ
                        Q2[s][a] = defaultQ
    ...
    def move2state(self, a, t):
        ...
        if len(self.moves) < self.nP:
            ...
            if self.nP == 2:
                if int(self.moves[0]) == 1 and int(self.moves[1]) == 1:
                    s = self.STATE_A
                elif int(self.moves[0]) == 1 and int(self.moves[1]) == -1:
                    s = self.STATE_B
                elif int(self.moves[0]) == -1 and int(self.moves[1]) == 1:
                    s = self.STATE_C
                elif int(self.moves[0]) == -1 and int(self.moves[1]) == -1:
                    ...
                    s = None
                    print('wrong state codes: ', s, a, self.moves, int(
                        self.moves[0]) == 1, int(self.moves[1]) == 1)
            else:
                if np.sum(self.moves) == len(self.moves):
                    s = self.STATE_A
                elif np.sum(self.moves) == -len(self.moves):
                    s = self.STATE_D
                elif 1 in self.moves and -1 in self.moves:
                    s = self.STATE_B
                else:
                    s = None
                    print('wrong state codes: ', s, a, self.moves, int(
    ...
    def selectInitialActionBatch(self, startState, veryFirst=False, last_a=None, Ns=None, NSAs=None, t=None):
        ...
        for i in np.arange(self.nP):
            a.append(self.selectInitialAction(startState, veryFirst,
                     last_a[i], Ns[i], NSAs[i], self.algs[i], i, self.fQprimes[i], t=t))
        return a
    ...
    def updateAgentBatch(self, s, a, r, nxt_s, alpha, t, pr=None, nr=None, Ns=None, NSAs=None):
        nxt_a = []
        for i, alg in enumerate(self.algs):
            nxt_a.append(self.updateAgent(
                s, a[i], r[i], nxt_s, alpha[i], t, pr[i], nr[i], Ns[i], NSAs[i], alg, i, self.fQprimes[i]))
        return nxt_a
    ...
    def experiment(self):
        ...
        # contains the number of times Cooperate action is chosen at initial state I
        IC = [0] * self.nP
        ...
        for i in range(self.T):
            ...
            if i == 0:
                a = self.selectInitialActionBatch(
                    self.initialState, True, last_a, Ns, NSAs, t=i)
            else:
                a = self.selectInitialActionBatch(
                    self.initialState, False, last_a, Ns, NSAs, t=i)
            ...
            while not gameover:
                ...
                # apply epsilon greedy selection (including for action chosen at STATE A)
                a = self.randomActionBatch(s, a, Ns, self.epsilon)
                ...
                # update the Q values and get the best action for the next state
                nxt_a = self.updateAgentBatch(
                    s, a, reward, nxt_s, alpha, t, pos_reward, neg_reward, Ns, NSAs)
            ...
            if not (i in episodes):
                ...
                # episodes[i]["Q1(I)c"] = episodes[i]["Q2(I)c"] = episodes[i]["Q1(I)d"] = episodes[i]["Q2(I)d"] = 0
            for p in np.arange(self.nP):
                episodes[i]["count"+str(p)], episodes[i]["percent" +
                                                         str(p)] = IC[p], IC[p] / (i+1)
                episodes[i]["reward"+str(p)], episodes[i]["pos_reward"+str(
                    p)], episodes[i]["neg_reward"+str(p)] = reward[p], pos_reward[p], neg_reward[p]
    ...
    def run(self):
        ...
        for k in range(self.nTrials):
            ...
            for i in range(self.T):
                for p in range(self.nP):
                    stats = agent_stats[p]
                    stats["count"][k, i] = tmp[i]["count" + str(p)]
                    stats["percent"][k, i] = 100 * \
                        tmp[i]["count" + str(p)] / (i + 1)
                    stats["reward"][k, i] = tmp[i]["reward" + str(p)]
        ...
        for p in range(self.nP):
            stats = agent_stats[p]
            report["count" + str(p)] = stats["count"]
            report["percent" + str(p)] = stats["percent"]
            report["reward" + str(p)] = stats["reward"]
            report["pos_reward" + str(p)] = stats["pos_reward"]
            report["neg_reward" + str(p)] = stats["neg_reward"]
            report["actions" + str(p)] = stats["actions"]
            report["cumreward" + str(p)] = stats["reward"]
